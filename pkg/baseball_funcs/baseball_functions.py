...
def walk_batter(bb_diamond_walk_runner):
    ...
    if base1_f == 1:
        if (
            base3_f == 1 and base2_f == 1
        ):  # Bases loaded, increment home plate for third base runner to home
            home_plate_f += 1

        if (
            base3_f == 1 and base2_f == 0
        ):  # Move runner to second base, third base stays
            base2_f = 1

        if (
            base3_f == 0 and base2_f == 1
        ):  # Move runner to third base, second base stays
            base3_f = 1
...
def at_bat(inning_data_func):
    ...
    while True:
        print(
            "At bat for {}: {}\n".format(
                team_description_func[team_at_bat_fa], players_tuple_func[batter_up_fa]
            )
        )
